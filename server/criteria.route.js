...
function makeCriteriaRoutes(db) {
  ...
  async function postCriteria(req, res) {
    try {
      ...
      if (row) {
        ...
        db.prepare('INSERT INTO criteria_checks (evaluation_id, conduct_id, tramo, criterion_index, is_checked) VALUES (?, ?, ?, ?, ?)').run(evaluationId, conductId, tramo, criterionIndex, isChecked ? 1 : 0);
      }
...
module.exports = { makeCriteriaRoutes }; 